...
class Dots:
    ...
    def eat(self, pacman_x, pacman_y):
        ...
        for i in list(self.top_row):
            if ((abs(i.x - pacman_x)) <= self.EAT_DIST and
               (abs(i.y - pacman_y)) <= self.EAT_DIST):
                self.top_row.remove(i)
        for i in list(self.bottom_row):
            if ((abs(i.x - pacman_x)) <= self.EAT_DIST and
               (abs(i.y - pacman_y)) <= self.EAT_DIST):
                self.bottom_row.remove(i)
        for i in list(self.left_col):
            if ((abs(i.x - pacman_x)) <= self.EAT_DIST and
               (abs(i.y - pacman_y)) <= self.EAT_DIST):
                self.left_col.remove(i)
        for i in list(self.right_col):
            if ((abs(i.x - pacman_x)) <= self.EAT_DIST and
               (abs(i.y - pacman_y)) <= self.EAT_DIST):
